import { strict as assert } from 'assert'
import { alignLines, findDelimiterIndex, resolveSpacing } from '../../../../src/features/editing/text/align-utils'

describe('alignLines', () => {
  it('should align assignment statements by "="', () => {
    const input = ['const FOO_BAR = 1;', 'const SHORT = 2;', 'const LONG_NAME = 3;']
    const expected = ['const FOO_BAR   = 1;', 'const SHORT     = 2;', 'const LONG_NAME = 3;']
    assert.deepEqual(alignLines(input, '='), expected)
  })

  it('should preserve leading indentation', () => {
    const input = ['  foo = 1;', '    bar = 2;', '  longer = 3;']
    const aligned = alignLines(input, '=')
    // Trimmed prefixes: "  foo" (5), "    bar" (7), "  longer" (8). Max 8, +1 space = col 9.
    assert.deepEqual(aligned, ['  foo    = 1;', '    bar  = 2;', '  longer = 3;'])
  })

  it('should leave lines without the delimiter untouched', () => {
    const input = ['foo = 1', '// just a comment', 'longer = 2']
    const aligned = alignLines(input, '=')
    assert.equal(aligned[1], '// just a comment')
    assert.equal(aligned[0], 'foo    = 1')
    assert.equal(aligned[2], 'longer = 2')
  })

  it('should return the input unchanged when fewer than two lines contain the delimiter', () => {
    const input = ['foo = 1', 'no delimiter here']
    assert.deepEqual(alignLines(input, '='), input)
  })

  it('should align by multi-character delimiters', () => {
    const input = ['x => 1', 'longerName => 2']
    const aligned = alignLines(input, '=>')
    assert.deepEqual(aligned, ['x          => 1', 'longerName => 2'])
  })

  it('should align line comments at the end of code lines', () => {
    const input = ['foo() // do foo', 'doSomethingLonger() // do it']
    const aligned = alignLines(input, '//')
    assert.deepEqual(aligned, ['foo()               // do foo', 'doSomethingLonger() // do it'])
  })

  it('should align by colon with no space before when configured', () => {
    const input = ['name: string', 'longField: number']
    const aligned = alignLines(input, ':', { spacesBefore: 0, spacesAfter: 1 })
    // Max trimmed prefix = "longField" (9). For "name" (4): padding = 9-4+0 = 5.
    assert.deepEqual(aligned, ['name     : string', 'longField: number'])
  })

  it('should trim leading whitespace from the suffix', () => {
    const input = ['a =    1', 'longer =   2']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['a      = 1', 'longer = 2'])
  })

  it('should not add trailing space when the suffix is empty', () => {
    const input = ['short =', 'longerOne =']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['short     =', 'longerOne ='])
  })

  it('should honor custom spacesBefore and spacesAfter', () => {
    const input = ['foo=1', 'bar=2', 'longer=3']
    const aligned = alignLines(input, '=', { spacesBefore: 2, spacesAfter: 3 })
    // Max trimmed prefix = "longer" (6). spacesBefore = 2.
    // foo (3): padding 6-3+2 = 5. bar (3): 5. longer (6): 2.
    assert.deepEqual(aligned, ['foo     =   1', 'bar     =   2', 'longer  =   3'])
  })

  it('should align by the first occurrence even when the delimiter appears multiple times', () => {
    const input = ['a = b = 1', 'longer = c = 2']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['a      = b = 1', 'longer = c = 2'])
  })

  it('should do nothing when the delimiter is an empty string', () => {
    const input = ['a = 1', 'b = 2']
    assert.deepEqual(alignLines(input, ''), input)
  })

  it('should handle a single-line input unchanged', () => {
    assert.deepEqual(alignLines(['foo = 1'], '='), ['foo = 1'])
  })

  it('should handle an empty input unchanged', () => {
    assert.deepEqual(alignLines([], '='), [])
  })

  it('should not mutate the input array', () => {
    const input = ['foo = 1', 'longer = 2']
    const copy = [...input]
    alignLines(input, '=')
    assert.deepEqual(input, copy)
  })

  it('should treat tabs as trimmable whitespace in the prefix', () => {
    const input = ['foo\t= 1', 'longer = 2']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['foo    = 1', 'longer = 2'])
  })

  it('should not split compound assignment operators when aligning by =', () => {
    const input = ['a = 1', 'total += 2']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['a = 1', 'total += 2'])
  })

  it('should leave comparison-only lines untouched when aligning by =', () => {
    const input = ['a = 1', 'if (x == y) {', 'b = 2']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['a = 1', 'if (x == y) {', 'b = 2'])
  })

  it('should not split arrow functions when aligning by =', () => {
    const input = ['const f = 1', 'map(v => v)']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['const f = 1', 'map(v => v)'])
  })

  it('should align by the assignment even when an arrow or comparison appears later in the line', () => {
    const input = ['const f = v => v', 'const longer = x == y']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['const f      = v => v', 'const longer = x == y'])
  })

  it('should not split logical and nullish compound assignments when aligning by =', () => {
    const input = ['a = 1', 'b ??= 2', 'c ||= 3', 'd &&= 4', 'e **= 5']
    const aligned = alignLines(input, '=')
    assert.deepEqual(aligned, ['a = 1', 'b ??= 2', 'c ||= 3', 'd &&= 4', 'e **= 5'])
  })
})

describe('findDelimiterIndex', () => {
  it('should find a bare assignment equals', () => {
    assert.equal(findDelimiterIndex('a = 1', '='), 2)
  })

  it('should skip == and === when looking for =', () => {
    assert.equal(findDelimiterIndex('if (a == b)', '='), -1)
    assert.equal(findDelimiterIndex('if (a === b)', '='), -1)
  })

  it('should skip compound operators when looking for =', () => {
    const lines = ['a += 1', 'a -= 1', 'a *= 1', 'a /= 1', 'a %= 1', 'a &= 1', 'a |= 1', 'a ^= 1', 'a <<= 1', 'a >>= 1', 'a != 1', 'a <= 1', 'a >= 1']
    for (const line of lines) {
      assert.equal(findDelimiterIndex(line, '='), -1, line)
    }
  })

  it('should skip arrows when looking for =', () => {
    assert.equal(findDelimiterIndex('map(v => v)', '='), -1)
  })

  it('should use plain indexOf for other delimiters', () => {
    assert.equal(findDelimiterIndex('a: b: c', ':'), 1)
    assert.equal(findDelimiterIndex('x => y', '=>'), 2)
  })

  it('should return -1 when the delimiter is absent', () => {
    assert.equal(findDelimiterIndex('no delimiter here', '='), -1)
  })
})

describe('resolveSpacing', () => {
  it('should return the per-delimiter value when present', () => {
    assert.equal(resolveSpacing({ default: 1, ':': 0 }, ':', 99), 0)
  })

  it('should fall back to "default" when the delimiter is not in the map', () => {
    assert.equal(resolveSpacing({ default: 2 }, '=', 99), 2)
  })

  it('should fall back to the hard-coded value when neither delimiter nor default exists', () => {
    assert.equal(resolveSpacing({}, '=', 7), 7)
  })

  it('should fall back to the hard-coded value when the map is undefined', () => {
    assert.equal(resolveSpacing(undefined, '=', 3), 3)
  })

  it('should ignore invalid (non-number or negative) entries', () => {
    assert.equal(resolveSpacing({ '=': -1 as unknown as number, default: 1 }, '=', 99), 1)
  })

  it('should accept zero as a valid value', () => {
    assert.equal(resolveSpacing({ ':': 0 }, ':', 99), 0)
  })
})
