...
function applyEdit(
  ...
  const before =
    lines.slice(0, edit.startLine).join('\n') +
    (edit.startLine > 0 ? '\n' : '') +
    lines[edit.startLine].substring(0, edit.startCol)
  const after =
    lines[edit.endLine].substring(edit.endCol) +
...
describe('findStatementEnd', () => {
  ...
  it('should handle semicolons inside parens', () => {
    assert.deepEqual(findStatementEnd(['for (let i = 0; i < n; i++) doSomething();'], 0, 28), { line: 0, col: 41 })
  })
  ...
  it('should handle object literals without semicolons', () => {
    assert.deepEqual(findStatementEnd(['return { a: 1 }'], 0, 0), { line: 0, col: 14 })
  })
})
...
describe('addBraces', () => {
  it('should add braces to single-line if with semicolons', () => {
    assert.equal(addBraces('if (cond) return value;', 0), 'if (cond) {\n  return value;\n}')
  })
  ...
  it('should add braces to multi-line if', () => {
    assert.equal(addBraces('if (cond)\n  return value', 1), 'if (cond) {\n  return value\n}')
  })

  it('should add braces to else', () => {
    assert.equal(addBraces('else return value', 0), 'else {\n  return value\n}')
  })
  ...
  it('should add braces to else if', () => {
    assert.equal(addBraces('else if (cond) return value', 0), 'else if (cond) {\n  return value\n}')
  })

  it('should add braces to for', () => {
    assert.equal(addBraces('for (const x of arr) doSomething(x)', 0), 'for (const x of arr) {\n  doSomething(x)\n}')
  })
  ...
  it('should handle function call without semicolons', () => {
    assert.equal(addBraces('if (cond) doSomething()', 0), 'if (cond) {\n  doSomething()\n}')
  })
...
describe('removeBraces', () => {
  it('should remove braces from multi-line block with semicolons', () => {
    assert.equal(removeBraces('if (cond) {\n  return value;\n}', 1), 'if (cond)\n  return value;')
  })
  ...
  it('should remove braces from single-line block', () => {
    assert.equal(removeBraces('if (cond) { return value; }', 0), 'if (cond)\n  return value;')
  })
