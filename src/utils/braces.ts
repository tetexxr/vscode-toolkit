...
export function findBracelessControl(
  lines: string[],
  cursorLine: number,
): BracelessControl | null {
...
export function findBracedSingleStatementControl(
  lines: string[],
  cursorLine: number,
): BracedControl | null {
  const minLine = Math.max(0, cursorLine - MAX_SEARCH_LINES);
  ...
  for (let line = cursorLine; line >= minLine; line--) {
    ...
    const stmtText = getTextRange(
      lines, stmtStart.line, stmtStart.col, stmtEnd.line, stmtEnd.col + 1,
    );
...
function parseControlKeyword(
  lines: string[],
  line: number,
): { indent: string; keyword: string; condEnd: Pos } | null {
...
export function nextNonWhitespace(
  lines: string[],
  line: number,
  col: number,
): (Pos & { ch: string }) | null {
...
export function findClosingParen(
  ...
  for (let l = line; l < lines.length; l++) {
    ...
    for (let i = l === line ? col : 0; i < text.length; i++) {
      ...
      if (ch === '(') depth++;
      else if (ch === ')') { depth--; if (depth === 0) return { line: l, col: i }; }
    }
  }
  return null;
}
