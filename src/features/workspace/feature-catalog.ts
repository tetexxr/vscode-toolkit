...
export const FEATURE_CATALOG: FeatureEntry[] = [
  ...
  { category: 'Git', name: 'Commit & Push — Selected Repositories', command: 'toolkit.git.commitPushAllRepos', description: "One commit message + push across every selected repository's staged changes; from the Source Control context menu. Commit-only, stage-all + commit, and stage-all + commit + push variants available" },
  { category: 'Git', name: 'Pull — Selected Repositories', command: 'toolkit.git.pullAllRepos', description: 'Pull every selected repository at once, skipping any without an upstream; from the Source Control context menu' },
  { category: 'Git', name: 'Synchronize — Selected Repositories', command: 'toolkit.git.syncAllRepos', description: 'Pull then push every selected repository at once (honours your pull.rebase config); from the Source Control context menu' },
  ...
  { category: 'Editing', name: 'Password Generator', command: 'toolkit.passwordGenerator.open', description: 'KeePassXC-style panel: secure (CSPRNG) passwords with class/length options and a live entropy strength meter; editable field re-estimates strength for passwords you type or paste' },
  ...
  { category: 'Workspace', name: 'Word Bookmarks', command: 'toolkit.docx.checkWorkspace', description: 'Scan .docx templates for bookmark problems in a filterable panel — chiefly a placeholder split across runs, which breaks first-run replacement, plus duplicate names, orphans and over-long names — and consolidate the fixable ones per row or all at once, keeping formatting' },
  { category: 'Workspace', name: 'PowerPoint Placeholders', command: 'toolkit.pptx.checkWorkspace', description: 'Scan .pptx templates for {{Placeholder}} problems in a filterable panel — one stored across several runs, which a run-by-run exporter would only half fill, a name that is not a single word, braces with no counterpart — and fix them per row or all at once: split ones merged into a single run, loose names canonicalised ({{ work centers }} to {{WorkCenters}}), keeping the formatting around them' },
  { category: 'Workspace', name: 'PDF Fields', command: 'toolkit.pdf.inspectFields', description: 'Inspect and edit the AcroForm fields of a PDF in a panel — set text, checkbox, radio, dropdown and list values inline (or clear them) and save all changes at once, overwriting the file while preserving each field styling' },
