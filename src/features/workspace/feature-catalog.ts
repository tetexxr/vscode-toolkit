...
export const FEATURE_CATALOG: FeatureEntry[] = [
  ...
  { category: 'Workspace', name: 'PowerPoint Placeholders', command: 'toolkit.pptx.checkWorkspace', description: 'Scan .pptx templates for {{Placeholder}} problems in a filterable panel — chiefly one stored across several runs, which a run-by-run exporter would only half fill, plus names that are not a single word and braces with no counterpart — and consolidate the fixable ones per row or all at once, keeping the formatting around them' },
