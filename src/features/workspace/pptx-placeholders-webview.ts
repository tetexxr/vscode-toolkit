/**
 * Generates the HTML/CSS/JS for the PowerPoint Placeholders panel — a flat,
 * filterable table with one row per placeholder and location across every
 * scanned .pptx, and a [Fix] action on the rows that can be consolidated.
 * Reuses the shared button styling and semantic palette so it matches the rest
 * of the toolkit's panels.
 */

import { cssColor } from '../../utils/palette'
import { BUTTON_CSS, BADGE_CSS } from '../../utils/webview-ui'

export function generatePlaceholdersHtml(nonce: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <title>PowerPoint Placeholders</title>
...
.cell-location { color: var(--vscode-descriptionForeground); white-space: nowrap; }
...
.uses { color: var(--vscode-descriptionForeground); font-size: 0.85rem; margin-left: 0.4em; }
...
(function () {
  ...
  const STATUS = {
    ...
    'crosses-break': { label: 'crosses break', cls: 'badge-warning' },
    'malformed':     { label: 'malformed',     cls: 'badge-warning' },
    'unclosed':      { label: 'unclosed',      cls: 'badge-error' }
  };
  ...
  function visibleRows() {
    ...
    if (q) {
      rows = rows.filter(function (r) {
        return (r.relPath + ' ' + r.name + ' ' + r.location).toLowerCase().indexOf(q) !== -1;
      });
    }
  ...
  function renderToolbar() {
    ...
    $toolbar.innerHTML =
      '<span class="toolbar-title">PowerPoint Placeholders</span>' +
      ...
      '<input id="search" class="search-box" type="search" placeholder="Filter by file, placeholder or slide…" value="' + esc(state.search) + '" />' +
  ...
  function renderSummary() {
    ...
    $summary.textContent =
      state.rows.length + ' placeholder(s) in ' + fileCount + ' file(s) · ' +
  ...
  function renderContent() {
    if (state.scanning) {
      $content.innerHTML = '<div class="empty-message"><span class="spinner"></span> Scanning presentations…</div>';
      return;
    }
    if (state.rows.length === 0) {
      $content.innerHTML = '<div class="empty-message">No .pptx files found in the scope.</div>';
      return;
    }
    const rows = visibleRows();
    if (rows.length === 0) {
      const msg = state.filter === 'issues' ? 'No placeholder issues — all clean 🎉' : 'Nothing matches the current filter.';
    ...
    let html = '<table><thead><tr>' +
      ...
      '<th data-sort="name">Placeholder' + arrow('name') + '</th>' +
      '<th>Location</th>' +
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const uses = r.uses > 1 ? '<span class="uses">×' + r.uses + '</span>' : '';
      html += '<tr>' +
        '<td><span class="cell-file truncate" data-file="' + esc(r.file) + '" title="' + esc(r.relPath) + '">' + esc(basename(r.relPath)) + '</span></td>' +
        '<td class="cell-name truncate" title="' + esc(r.name) + '">' + esc(r.name) + uses + '</td>' +
