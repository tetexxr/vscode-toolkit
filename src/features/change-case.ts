...
import {
  toCamelCase, toSnakeCase, toPascalCase, toConstantCase,
  toKebabCase, toTitleCase, toLowerCase, toUpperCase,
  toDotCase, toPathCase, toSentenceCase, toSwapCase, toNoCase,
} from '../utils/text';
...
const CASE_DEFINITIONS: CaseDefinition[] = [
  ...
  { command: 'toolkit.changeCaseConstant', label: 'CONSTANT_CASE',  description: 'Convert to CONSTANT_CASE',  fn: toConstantCase },
  ...
  { command: 'toolkit.changeCaseSentence', label: 'Sentence case',  description: 'Convert to Sentence case',  fn: toSentenceCase },
...
export function registerChangeCaseCommands(context: vscode.ExtensionContext): void {
  ...
  for (const def of CASE_DEFINITIONS) {
    context.subscriptions.push(
      vscode.commands.registerCommand(def.command, () => applyTransformation(def.fn))
    );
  }
    ...
    vscode.commands.registerCommand('toolkit.changeCase', async () => {
      ...
      if (editor && editor.selections.length === 1) {
        ...
        if (sel.isEmpty) {
          ...
          if (expanded) { range = expanded; } else { range = sel; }
        } else {
          range = sel;
        }
