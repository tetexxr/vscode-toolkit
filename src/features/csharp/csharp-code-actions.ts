...
const CLASS_RE =
  /(?:public|internal|private|protected)\s+(?:(?:static|partial|sealed|abstract)\s+)*class\s+(\w+)/g;
...
export class CSharpCodeActionProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    const ctorAction = new vscode.CodeAction(
  ...
  if (expressionBodied) {
    if (properties.length === 1) {
      ...
      const targets = properties
        .map(p => (useThis ? `this.${p.name}` : `${prefix}${p.name}`))
        .join(', ');
