/**
 * Generates the HTML/CSS/JS for the npm Workspace Overview webview.
...
<head>
  ...
  <title>npm Workspace Overview</title>
...
.pkg-table col.col-name    { width: 32%; }
.pkg-table col.col-type    { width: 10%; }
.pkg-table col.col-version { width: 16%; }
.pkg-table col.col-latest  { width: 16%; }
.pkg-table col.col-status  { width: 12%; }
.pkg-table col.col-actions { width: 14%; }
...
(function() {
  ...
  function getOutdatedPackages() {
    ...
    for (var i = 0; i < state.projects.length; i++) {
      ...
      for (var j = 0; j < proj.packages.length; j++) {
        ...
        if (pkg.isOutdated) {
          pkgs.push({ projectFsPath: proj.fsPath, packageName: pkg.name, version: pkg.latestVersion, devDependency: pkg.dependencyType === 'devDependencies' });
        }
      }
    }
    return pkgs;
  ...
  function renderContent() {
    if (!state.projects || state.projects.length === 0) {
      $content.innerHTML = '<div class="empty-message">No Node.js projects found in workspace.</div>';
      return;
    }
    ...
    for (const proj of state.projects) {
      ...
      for (const pkg of filtered) {
        ...
        if (pkg.isOutdated) {
          html += '<button class="btn btn-update-sm" data-project="' + esc(proj.fsPath) + '" data-pkg="' + esc(pkg.name) + '" data-ver="' + esc(pkg.latestVersion) + '" data-dev="' + (pkg.dependencyType === 'devDependencies' ? '1' : '0') + '">Update</button>';
        }
        html += '</td>';
        html += '</tr>';
      }
    ...
    $content.querySelectorAll('[data-pkg]').forEach(btn => {
      btn.addEventListener('click', () => {
        post({
          ...
          packageName: btn.dataset.pkg,
          version: btn.dataset.ver,
          devDependency: btn.dataset.dev === '1',
        });
      });
    });
  }
