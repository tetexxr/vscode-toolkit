...
| PowerPoint Placeholders | — | Check .pptx templates for `{{Placeholder}}` problems in a filterable panel — a placeholder stored across several runs (which a run-by-run exporter would only half fill), a name that is not a single word, braces with no counterpart. Run from the palette (whole workspace) or the explorer context menu (PowerPoint ▸ Check / Fix Placeholders); fix them per row or all at once, in place — split ones merged into a single run, loose names canonicalised (`{{ work centers }}` → `{{WorkCenters}}`), keeping the formatting around them |
